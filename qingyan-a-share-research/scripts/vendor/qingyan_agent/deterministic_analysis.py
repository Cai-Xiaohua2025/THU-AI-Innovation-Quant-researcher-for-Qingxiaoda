...
def _compose_detailed_answer(
    ...
    if backtest:
        ...
        lines.append(
            f"- **样本期**：原始K线 {metrics.get('sample_start_date') or '待核验'} 至 "
            f"{metrics.get('sample_end_date') or '待核验'}，共 {metrics.get('sample_rows') or 0} 根；"
            f"扣除 {metrics.get('signal_warmup_bars') or 0} 根均线预热后，有效模拟期 "
            f"{metrics.get('simulation_start_date') or '待核验'} 至 "
            f"{metrics.get('simulation_end_date') or '待核验'}。"
        )
...
def _compose_standard_answer(
    ...
    *,
    ...
    if backtest:
        ...
        lines.append(
            f"- 原始K线样本期 {metrics.get('sample_start_date') or '待核验'} 至 "
            f"{metrics.get('sample_end_date') or '待核验'}，共 {metrics.get('sample_rows') or 0} 根；"
            f"扣除 {metrics.get('signal_warmup_bars') or 0} 根均线预热后，有效模拟期 "
            f"{metrics.get('simulation_start_date') or '待核验'} 至 "
            f"{metrics.get('simulation_end_date') or '待核验'}。"
